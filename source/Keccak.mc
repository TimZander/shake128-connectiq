...
class Keccak {
    ...
    // Each constant is [hi, lo] representing a 64-bit value
    private static const RC as Array< Array<Number> > = [
        [0x00000000, 0x00000001],  // RC[0]
        [0x00000000, 0x00008082],  // RC[1]
        [0x80000000, 0x0000808A],  // RC[2]
        [0x80000000, 0x80008000],  // RC[3]
        [0x00000000, 0x0000808B],  // RC[4]
        [0x00000000, 0x80000001],  // RC[5]
        [0x80000000, 0x80008081],  // RC[6]
        [0x80000000, 0x00008009],  // RC[7]
        [0x00000000, 0x0000008A],  // RC[8]
        [0x00000000, 0x00000088],  // RC[9]
        [0x00000000, 0x80008009],  // RC[10]
        [0x00000000, 0x8000000A],  // RC[11]
        [0x00000000, 0x8000808B],  // RC[12]
        [0x80000000, 0x0000008B],  // RC[13]
        [0x80000000, 0x00008089],  // RC[14]
        [0x80000000, 0x00008003],  // RC[15]
        [0x80000000, 0x00008002],  // RC[16]
        [0x80000000, 0x00000080],  // RC[17]
        [0x00000000, 0x0000800A],  // RC[18]
        [0x80000000, 0x8000000A],  // RC[19]
        [0x80000000, 0x80008081],  // RC[20]
        [0x80000000, 0x00008080],  // RC[21]
        [0x00000000, 0x80000001],  // RC[22]
        [0x80000000, 0x80008008],  // RC[23]
    ];
    ...
    public function permute() as Void {
        ...
        var C_hi = new Array<Number>[5];  // Column parity (hi words)
        var C_lo = new Array<Number>[5];  // Column parity (lo words)
        var D_hi = new Array<Number>[5];  // D values (hi words)
        var D_lo = new Array<Number>[5];  // D values (lo words)
        var B_hi = new Array<Number>[25]; // After rho+pi (hi words)
        var B_lo = new Array<Number>[25]; // After rho+pi (lo words)
        ...
        for (var round = 0; round < 24; round++) {
            ...
            for (var x = 0; x < 5; x++) {
                ...
                var rotHi = (C_hi[x2] << 1) | (ushr(C_lo[x2], 31) & 1);
                var rotLo = (C_lo[x2] << 1) | (ushr(C_hi[x2], 31) & 1);
            ...
            for (var i = 0; i < 25; i++) {
                ...
                if (r == 0) {
                    ...
                    var newHi = (hi << r) | (ushr(lo, 32 - r) & ((1 << r) - 1));
                    var newLo = (lo << r) | (ushr(hi, 32 - r) & ((1 << r) - 1));
                    hi = newHi;
                    lo = newLo;
                } else {
                    // r > 32
                    var s = r - 32;
                    var newHi = (lo << s) | (ushr(hi, 32 - s) & ((1 << s) - 1));
                    var newLo = (hi << s) | (ushr(lo, 32 - s) & ((1 << s) - 1));
                    hi = newHi;
                    lo = newLo;
                }
            ...
            // === Iota step ===
            // A[0,0] ^= RC[round]
            state[0] = state[0] ^ RC[round][0];
            state[1] = state[1] ^ RC[round][1];
        }
    }
    ...
    public function extractBytes(output as ByteArray, offset as Number, len as Number, stateOffset as Number) as Void {
        for (var i = 0; i < len; i++) {
            var bytePos = stateOffset + i;
            var laneIdx = bytePos / 8;
            var byteInLane = bytePos % 8;
